...
syllabi <- readRDS(filename_data) %>%
    dplyr::filter(nchar(team) < 3) %>%
    dplyr::select(field, year, team = team_ordered, team_rand, country) %>% 
    tidyr::pivot_longer(
...
ds_count <- syllabi %>%
    dplyr::count(formation, composition, field, year, country) %>% 
    mutate(
...
p_country <- d_country_yr %>%
    ...
    geom_text_repel(
        ...
        aes(label = country), nudge_x = 3.5, direction = "y",
        segment.colour = "gray75",
        hjust = "left",
        show.legend = FALSE,
        size = 3,
    ) +
    geom_point(data = filter(d_country_yr, year == 2019), show.legend = FALSE) +
    labs(
        x = "Year", y = "Syllabi"
    )
...
p_field <- d_field %>%
    ggplot(aes(x = year, y = n, group = field, color = field)) +
    geom_line(linewidth = 0.5, show.legend = FALSE) +
    # scale_y_log10(
    #     breaks = scales::log_breaks(),
    #     labels = scales::label_number()
    ...
    geom_text_repel(
        data = ~ filter(.x, year == max(year)),
        aes(label = field), nudge_x = 3.5, direction = "y",
        segment.colour = "gray75",
        hjust = "left",
        show.legend = FALSE,
        size = 3,
    ) +
    geom_point(data = ~ filter(.x, year == 2019), show.legend = FALSE) +
    labs(
        x = "Year", y = "Syllabi"
    )
...
d_teams <- syllabi %>%
    filter(formation == "team") %>%
    count(year, composition)

p_teams <- d_teams %>%
    ggplot(aes(x = year, y = n, group = composition, color = composition)) +
    geom_line(linewidth = 0.5, show.legend = FALSE) +
    scale_y_log10(
        breaks = scales::log_breaks(),
        labels = scales::label_number()
    ...
    geom_text_repel(
        data = ~ filter(.x, year == max(year)),
        aes(label = composition), nudge_x = 3.5, direction = "y",
        segment.colour = "gray75",
        hjust = "left",
        show.legend = FALSE,
        size = 3,
...
tbl_country <- ds %>%
    ...
    dplyr::bind_rows(., mutate(., country_name = "All")) %>%
    dplyr::count(country_name, wt = n, sort = TRUE) %>%
    dplyr::mutate(prop = 100 * n / sum(n[country_name != "All"]))
